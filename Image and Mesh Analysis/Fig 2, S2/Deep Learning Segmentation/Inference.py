...
trained_model = "./models/model295.pth"

patch_size = (32, 128, 128)
stride=(16, 64, 64)
# Process XY plane
preprocessor = Preprocess(patch_size=patch_size, stride=stride)
swapped_image = preprocessor.load_and_swap_axes(input_tomogram, axis_order=(0, 1, 2))
...
inference = Inference(model_path=trained_model, device='cuda')
...
unswapped_image = postprocessor.unswap_axes_and_save(unpadded_image, axis_order=(0, 1, 2), output_filepath = f"./inference_output/{input_tomogram}_XY.tif")
...
# Process XZ plane
preprocessor = Preprocess(patch_size=patch_size, stride=stride)
swapped_image = preprocessor.load_and_swap_axes(input_tomogram, axis_order=(1, 0, 2))
padded_image, original_shape, pad_width = preprocessor.center_pad_image(swapped_image)
patches, positions = preprocessor.extract_patches(padded_image)
...
reassembled_image = postprocessor.reassemble_patches(predictions, positions)
unpadded_image = postprocessor.unpad_image(reassembled_image, original_shape, pad_width)
unswapped_image = postprocessor.unswap_axes_and_save(unpadded_image, axis_order=(1, 0, 2), output_filepath = f"./inference_output/{input_tomogram}_XZ.tif")

patch_size = (128, 128, 32)
stride=(64, 64, 16)
# Process YZ plane
preprocessor = Preprocess(patch_size=patch_size, stride=stride)
swapped_image = preprocessor.load_and_swap_axes(input_tomogram, axis_order=(2, 1, 0))
padded_image, original_shape, pad_width = preprocessor.center_pad_image(swapped_image)
patches, positions = preprocessor.extract_patches(padded_image)
...
reassembled_image = postprocessor.reassemble_patches(predictions, positions)
unpadded_image = postprocessor.unpad_image(reassembled_image, original_shape, pad_width)
unswapped_image = postprocessor.unswap_axes_and_save(unpadded_image, axis_order=(2, 1, 0), output_filepath = f"./inference_output/{input_tomogram}_YZ.tif")

# Merge all predictions
filepaths = ["./inference_output/Control4_XY.tif", "./inference_output/Control4_XZ.tif", "./inference_output/Control4_YZ.tif"]
